	...
	HRESULT __stdcall NewWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		try
		{
			...
			case WM_XBUTTONUP:
			{
				...
				if (LuaState::gHooks.HasHook(LuaHooks::HookType::HOOK_MOUSE))
				{
					auto v = LuaState::gHooks.GetCallbacks(LuaHooks::HookType::HOOK_MOUSE);
					result = luabind::call_function<bool>(v.front(), Events::EventMouse(eventType, btn, x, y));
				}
			}
			break;
